...
if (isset ($_GET["Buscar"]) && $_GET["Buscar"] == "Buscar")
{
    ...
    if (isset($_GET["Criterio"])) { $criterio = $_GET["Criterio"]; } else { $criterio = ""; }

    if ($criterio == "CodProducto")
    {
		$c_criterio = "CodProducto LIKE '%$claves%'";
	}
    else
    {
		$c_criterio = "Nombre LIKE '%$claves%'";
	}

    $sql = "SELECT * FROM productos WHERE $c_criterio ORDER BY $c_criterio";
	$Param = "&Buscar="."Buscar"."&Criterio=".$criterio."&Claves=".$claves."&pagina=".$pagina."&cant_reg=".$cant_reg."&cant_pag=".$cant_pag."&Pagina="."productos_bus.php";
    ...
    <div class="container" style="background-color: #FFF;">
        ...
        <div class="row">
            <div align="center" class="col-md-12">
                <h1>B&uacute;squeda de Productos</h1>
            </div>
        </div>
        ...
        <div class="row">
            <div align="center" class="col-md-12">
                <form name="frmBuscar" method="get" action="productos_bus.php">
                    <p>Elija un criterio para buscar, escriba las claves de b&uacute;squeda. Luego haga clic en buscar.</p>
                    <div class="form-group col-md-6">
                        <label for="Criterio">Seleccione criterio</label><br>
                        <select name="Criterio" id="Criterio" class="form-control">
                            <option value="CodProducto">C&oacute;d. de Producto</option>
                            <option value="Nombre" selected>Nombre de Producto</option>
                        </select>
                    </div>
                    
                    <div class="form-group col-md-6">
                        <label for="Claves">Escriba clave de b&uacute;squeda</label><br>
                        <input name="Claves" id="Claves" type="text" class="form-control" required>
                    </div>
        ...
        <div class="row">
            <div align="center" class="col-md-12">
                ...
                if ($pagina > 1)
                {
                    ...
                    $pag_ant = $pagina - 1;
                    echo "<a href='productos_bus.php?pagina=$pag_ant&cant_reg=$cant_reg&cant_pag=$cant_pag&Buscar=Buscar&Criterio=$criterio&Claves=$claves'>
                    <button name='cmdAnterior' type='button' class='btn btn-success' value='Anterior'>Anterior</button>
                    </a>";
                }
                $pagina ++;	
                ...
                if ($cant_pag >= $pagina)
                {
                    ...
                    echo "<a href='productos_bus.php?pagina=$pagina&cant_reg=$cant_reg&cant_pag=$cant_pag&Buscar=Buscar&Criterio=$criterio&Claves=$claves'>
                    <button name='cmdSiguiente' type='button' class='btn btn-success' value='Siguiente'>Siguiente</button>
                    </a>";
                }
                ?>
                </p>

                ...
                <table>
                    ...
                    <tbody>
                        ...
                        if ($results->num_rows > 0):
                            ...
                            <tr>
                                <td colspan="5">
                                    <h3>No existen productos en el carrito</h3>
                                </td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                    ...
                    <div class="shoping__discount shoping__checkout mt-0">
                        ...
                        <?php if (!isset($_SESSION['user'])): ?>
                            <div class="alert alert-danger mb-0" role="alert">
                                <i class="fa fa-exclamation-circle"></i> Si ya sos cliente, ingresa con tu usuario y accede a
                                nuestros descuentos por compra mayorista.
                            </div>
                        <?php endif; ?>
                    </div>
                <?php endif; ?>
        ...
        <div class="col-lg-6">
            <div class="shoping__checkout mt-0">
                ...
                <ul>
                    ...
                    if (!empty($data[0]) && !isset($_SESSION['user'])):
                        foreach ($data as $key => $value) {
                            if ($pedido->getTotalFinal() > $value['precio']):
                                ...
                                <li>Descuento <?= $value['descuento'] . "%"; ?> <span>-
                                        $<?php echo number_format($descuento, 2, ',', '.'); ?></span></li>
                                <?php break;
                            endif;
                        }
                    endif;
                    ...
                    <li>Total
                        <span>$<?php echo number_format($pedido->getTotalFinal() - $descuento, 2, ',', '.'); ?></span>
                    </li>
                </ul>
                ...
                <?php if ($pedido->getTotalFinal() < $general->getMinimo() && !isset($_SESSION['user'])): ?>
                    <div class="alert alert-danger" role="alert">
                        <i class="fa fa-exclamation-circle"></i> El minimo de compra para cerrar el pedido es de
                        <strong>$<?php echo number_format($general->getMinimo(), 0, '', '.'); ?></strong>
                    </div>
                <?php endif; ?>

                <!-- Final Order -->
                <?php if ((!$result['cpanel'] && $pedido->getTotalFinal() >= $general->getMinimo()) || isset($_SESSION['user'])): ?>
                    <a href="#" id="js-finally-order" data-id="<?php echo $result['Id_Pedido']; ?>"
                        class="primary-btn">Finalizar Pedido</a>
                <?php endif; ?>
            </div>
        </div>
    </div>
